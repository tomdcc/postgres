...
ReindexMultipleTables(const ReindexStmt *stmt, const ReindexParams *params)
{
	 ...
	 * as the ATTACH PARTITION path does.  That new transaction has no active
	...
	if (partedidxs != NIL)
	{
		...
		while (progress)
		{
			...
			foreach(lc, partedidxs)
			{
				...
				{
					progress = true;
					ReindexReportValidity(idxoid, true, params);
				}
		...
		/*
		 * Report what is still invalid only once the passes have converged:
		 * an index passed over on one pass may well be validated by the next,
		 * so only the state settled on is worth reporting.
		 */
		if ((params->options & REINDEXOPT_VERBOSE) != 0)
		{
			...
			foreach(lc, partedidxs)
			{
				...
				if (!SearchSysCacheExists1(RELOID, ObjectIdGetDatum(idxoid)))
					continue;
				if (!get_index_isvalid(idxoid))
					ReindexReportValidity(idxoid, false, params);
			}
		}
