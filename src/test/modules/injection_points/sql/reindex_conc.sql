...
-- The same applies to the other place an index's life ends without its row
-- going away: DROP INDEX CONCURRENTLY commits the dead state before its last
-- wait, so a failure after that leaves the dead row behind.  Clearing
-- indisnodata is what keeps pg_dump from restoring the index the user asked to
-- drop.  The plain index is the control: it is left INVALID, and this one must
-- not be reported any differently.
SELECT injection_points_attach('drop-index-concurrently-after-set-dead', 'error');
CREATE TABLE drop_nodata_tbl (i int);
INSERT INTO drop_nodata_tbl SELECT generate_series(1, 10);
CREATE INDEX drop_nodata_ind ON drop_nodata_tbl (i) WITH NO DATA;
CREATE INDEX drop_plain_ind ON drop_nodata_tbl (i);
DROP INDEX CONCURRENTLY drop_nodata_ind;
DROP INDEX CONCURRENTLY drop_plain_ind;
SELECT c.relname, i.indisready, i.indisvalid, i.indislive, i.indisnodata
  FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
  WHERE c.relname LIKE 'drop_%_ind'
  ORDER BY c.relname;
SELECT injection_points_detach('drop-index-concurrently-after-set-dead');
DROP TABLE drop_nodata_tbl;
