...
-- The same principle on a partitioned tree, which reaches the populated state
-- by a different route: REINDEX INDEX populates the leaves, and re-attaching an
-- already-attached child re-runs the check that makes the parent valid.  A
-- partitioned index has no storage of its own, so indisready is true from the
-- start and the deferral must leave no trace once both steps have run.
-- pg_dump depends on that: it selects on indisready, so a partitioned index
-- left unready would be silently omitted from the dump.  The tree is left
-- behind for pg_upgrade's regression fixture to carry.
CREATE TABLE nodata_psame (a int, b text) PARTITION BY RANGE (a);
CREATE TABLE nodata_psame1 PARTITION OF nodata_psame FOR VALUES FROM (0) TO (100);
INSERT INTO nodata_psame SELECT g, g::text FROM generate_series(0, 99) g;
CREATE UNIQUE INDEX nodata_psame_idx ON nodata_psame (a) INCLUDE (b) WITH NO DATA;
SELECT c.relname, i.indisready, i.indisvalid, i.indisnodata FROM pg_index i
  JOIN pg_class c ON c.oid = i.indexrelid
  WHERE c.relname LIKE 'nodata_psame%' ORDER BY 1;
REINDEX INDEX nodata_psame_idx;
SELECT format('ALTER INDEX nodata_psame_idx ATTACH PARTITION %s',
              inh.inhrelid::regclass)
  FROM pg_inherits inh WHERE inh.inhparent = 'nodata_psame_idx'::regclass \gexec
-- The control is the same tree built without the clause.
CREATE TABLE nodata_pctl (a int, b text) PARTITION BY RANGE (a);
CREATE TABLE nodata_pctl1 PARTITION OF nodata_pctl FOR VALUES FROM (0) TO (100);
INSERT INTO nodata_pctl SELECT g, g::text FROM generate_series(0, 99) g;
CREATE UNIQUE INDEX nodata_pctl_idx ON nodata_pctl (a) INCLUDE (b);
SELECT (SELECT ROW(indnatts, indnkeyatts, indisunique, indnullsnotdistinct,
                   indisprimary, indisexclusion, indimmediate, indisclustered,
                   indisvalid, indcheckxmin, indisready, indislive,
                   indisreplident, indisnodata, indkey::text,
                   indcollation::text, indclass::text, indoption::text,
                   indexprs::text, indpred::text)
          FROM pg_index WHERE indexrelid = 'nodata_pctl_idx'::regclass)
     = (SELECT ROW(indnatts, indnkeyatts, indisunique, indnullsnotdistinct,
                   indisprimary, indisexclusion, indimmediate, indisclustered,
                   indisvalid, indcheckxmin, indisready, indislive,
                   indisreplident, indisnodata, indkey::text,
                   indcollation::text, indclass::text, indoption::text,
                   indexprs::text, indpred::text)
          FROM pg_index WHERE indexrelid = 'nodata_psame_idx'::regclass)
    AS pg_index_rows_identical;
DROP TABLE nodata_pctl;
